...
> 最新验收：用户已通过 AgentMeter 实际发送，23:19 原会话在额度中断后产生新的“继续”消息并恢复助手及工具活动。手动按钮链路已验证，自动实时额度调度仍未开放。最新界面增加候选填入与持久入队历史，30 项发送/观察/监测回归测试通过。详细证据见 [queue 实测](CODEX_DESKTOP_QUEUE_RESEARCH.md)。

> 手动发送入口已接入：设置 → ChatGPT → “通过 Desktop 队列恢复一次”，支持原线程 UUID、额度中断文件复核、独立入队记录及防重复提交。实时额度/运行时状态仍需用户确认，自动调度保持关闭。11 项发送与观察测试通过；新按钮的 UI 实机验收未完成。详见 [queue 实现与限制](CODEX_DESKTOP_QUEUE_RESEARCH.md)。
...
2026-09-16：用户反馈手动恢复验证界面显示“已验证原会话收到‘继续’并产生新轮次及助手活动”。此为用户实机反馈，未提供对应测试轮次 ID，不扩大为自动恢复端到端通过。新增 Core `CodexResumeExecutor`：原 Desktop 归属及实时策略复核 → 持久化 attempting → 调用发送接口 → 校验线程与新轮次并持久化 submitted → 验证对应轮次执行。写盘失败锁住执行器；提交后的异常/取消/回执丢失持久化 uncertain，不重发；明确未发送才记录 failed。操作 ID 只用于关联记录，不宣称服务端幂等。

该执行器只接入了模拟传输测试，尚未绑定 Mac coordinator 或任何生产发送端，也不对用户开放发送按钮。`CodexResumeTransport` 实现必须证明原执行端归属，保持原设置，只发送“继续”，对状态变化拒绝发送并提供有界超时；普通 read 后 start 仍有并发竞态，不能宣称条件式提交保证。后续接入时必须由单一队列所有者协调监测、取消和 checkpoint，不能为同一存储创建多个执行器。运行时中断证据绑定仍未实现，本地候选保持不可发送。

本轮重新检查宿主内置 CLI 的公开 help 和默认 socket：支持 proxy，但默认控制目录/socket 仍不存在；没有启动 daemon、访问私有 IPC 或注入 UI。Core `CodexResumeExecutorTests` 与 `CodexResumePolicyTests` 合计 22 项通过，涵盖发送前落盘、保存失败、回执丢失、重启恢复、身份/额度/轮次不符、取消和重入。

2026-09-15 手动观察验证版：设置中的“手动恢复验证”允许输入测试线程 UUID，选择该线程的 JSONL 文件，在建立 EOF 基线后观察 60 秒。用户自行在 Codex 发送一次精确的“继续”。验证器要求新增用户消息、新 `task_started` 和随后的助手输出；历史消息、缺失时间戳、子串匹配不构成成功。重复消息、其他用户输入、收尾/中断或读取异常保守停止；超时为待确认。该入口不发送输入、不打开线程、不调用 CLI，也不修改自动恢复队列；它仅验证用户手动操作的效果，无法证明独立发送器可用。当前环境拒绝 Codex UI 访问，因此没有通过运行 CGEvent 或其他工具规避限制。

使用方法：打开设置 → ChatGPT → 手动恢复验证，填入允许继续的空闲测试线程 ID，选择其 rollout JSONL，等候“已开始观察”后切到原线程手动发送。观察结束后点击“停止观察”可重新选择。无需刻意耗尽额度；此步骤不能替代真实额度恢复与独立发送端的完整验收。仅对 Desktop 来源文件启用验证，持有文件句柄并校验已读尾部，单次读取上限 1 MiB，不保存正文。暂停或离开页面会取消观察，应用重启不会继续观察或发送任何内容。
...
## 2026-09-17：恢复列表、暂停与通知

设置 → ChatGPT → Codex 自动恢复现在提供统一开关、待恢复会话、原会话跳转、手动恢复确认，以及折叠的处理历史和高级诊断。首次开启解释范围；暂停保留等待项，重新开启建立新的 EOF 基线，不补录暂停期间的中断。不同会话互不淘汰，同会话只保留最新待处理失败。

`CodexResumeCoordinator` 统一管理扫描、60 秒重查、唤醒复核、发送、队列 checkpoint 和通知。重置时间只作为重查时间（加 20 秒），不会绕过原有额度 60 秒、会话 15 秒的新鲜度策略。自动与手动发送共享按 thread/failed-turn 独占创建的尝试文件。入队回执保存为 `queuedMessageID`，不作为 `submittedTurnID`；观察到后续活动单独显示 `observed`，不冒充严格关联成功。

中断、观察结果/确认结果、需处理异常分别按 10 秒窗口合并通知，去重事件保存在 checkpoint。拒绝系统通知不阻塞恢复列表。通知可打开 ChatGPT 设置的恢复列表。新开关使用 `codexAutomaticResumeEnabled`，不会从旧监测开关迁移自动发送授权。

**能力边界：** 2026-09-16 23:19 用户通过 AgentMeter 恢复原会话的真实验证仍然有效，生产手动入口继续使用已验证的 Desktop queue 路径。本轮没有重复向真实会话发消息。目前生产只读协议仍不能提供原 Desktop 归属、账号绑定和最后失败轮次的完整证据，因此自动发送适配器尚未安装；页面明确展示这个限制及可操作原因。调度器的有效证据分支通过注入 transport 测试，不等于无人值守自动恢复已经实机通过。禁止把当前只读 probe 或显示额度快照当作完整发送授权。

验证：新增覆盖多会话排序、暂停/重新建立基线、旧开关迁移、额度等待后强制刷新、核验期间暂停、旧手动 reservation 与自动发送冲突、通知合并去重，以及 queued message 与执行轮次分离。Mac 相关测试 47 项、Core 恢复测试 26 项通过，Mac 构建通过。界面工具未能连接临时构建，视觉实机验收未完成；真实自动触发仍需宿主提供完整核验证据后验收。
