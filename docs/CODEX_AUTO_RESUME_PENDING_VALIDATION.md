...
### 2026-09-16：AgentMeter 手动按钮真实额度中断验收

用户通过 AgentMeter 发送“继续”，已核对两条持久入队记录与原线程事件：22:30 的新轮次仍限额失败，23:19 的新轮次恢复助手和工具活动。手动按钮路径现有真实验证证据，详见 [queue 实测](CODEX_DESKTOP_QUEUE_RESEARCH.md)。自动实时额度复核、严格消息 ID 关联和无人值守整体验收仍未通过。本轮增加候选填入及重启后可见的入队历史，未新增发送。

### 2026-09-16：公开 queue 命令实测成功

Desktop 保持运行时，使用宿主 CLI `codex queue` 提交一次无工具测试消息；原线程新轮次完成且返回精确测试文字。详见 [实测证据及接入计划](CODEX_DESKTOP_QUEUE_RESEARCH.md)。此前“无独立发送入口”的判断需更新：已找到并实测入口，但生产适配器、入队回执关联、实时额度绑定、崩溃去重及自然额度中断验收仍未完成。后续不以公开 socket 可用作为 queue 方案的前置条件。
