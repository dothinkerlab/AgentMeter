...
### 用户通过 AgentMeter 按钮完成真实恢复

2026-09-16 用户反馈“已经通过 AgentMeter 发送继续”后，本轮只读复核了真实 queue-attempts 和原会话事件，没有再次发送。

- 目标为当前会话 `01a0a79c-66e8-70b2-afde-324da9570a11`（规划 Codex 自动恢复功能 (3)）。
- 第一次失败轮次 `01a0aa94-d966-7d80-90b2-712a67bedb20`，尝试记录 queued message ID 为 `01a0aaa0-061c-7413-a1d6-1f2c58e7b227`。北京时间 22:30:17.567 收到精确“继续”，新轮次 `01a0aaa0-2696-70d2-87b4-3269c8131fba` 在 22:30:28.809 再次以 usage_limit_exceeded 结束。
- 第二次针对上述新的失败轮次，queued message ID 为 `01a0aacd-5c33-7073-865a-365d01a1ae5a`。23:19:45.578 新轮次 `01a0aacd-709c-79f2-9d7b-2a0f1b67e24f` 启动，23:19:49.576 收到精确“继续”，之后恢复助手响应和工具执行，即本轮正在进行的开发。
- 两个记录针对不同失败轮次，并非对同一轮次重复提交。记录均为 queued；不把该状态本身等同于执行成功。
- 用户按钮操作 + 本地入队回执 + 同线程后续事件共同证明手动发送路径已在真实额度中断后工作。由于 rollout 未暴露 queued message ID，严格逐消息关联仍未成立，不能把这一结果扩大为自动额度调度通过。

界面进一步支持从最新监测候选填入线程及唯一来源文件，仅填入、不自动发送。新增最近 5 条入队记录，最多检查 500 个目录项；损坏记录提示异常并继续保留去重保护。记录在重启后仍显示，历史 attempting/uncertain 明确显示结果待确认。
